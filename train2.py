...
import dnnlib
from training import training_loop, training_loop2
...
def subprocess_fn(rank, c, temp_dir):
    ...
    training_loop2.training_loop(rank=rank, **c)
...
def init_dataset_kwargs(data, bbox_dim, aug=True):
    try:
        dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=data, bbox_dim=bbox_dim, use_labels=True, max_size=None, xflip=False, aug=aug)
        dataset_obj = dnnlib.util.construct_class_by_name(**dataset_kwargs) # Subclass of training.dataset.Dataset.
        dataset_kwargs.resolution = dataset_obj.resolution # Be explicit about resolution.
        dataset_kwargs.use_labels = dataset_obj.has_labels # Be explicit about labels.
        dataset_kwargs.max_size = len(dataset_obj) # Be explicit about dataset size.
        print(dataset_kwargs, dataset_obj.name)
...
@click.option('--data2',        help='Training data', metavar='[ZIP|DIR]',                      type=str, required=True)
...
@click.option('--bbox-dim',     help='Limit max objects num of per images', metavar='INT',      type=click.IntRange(min=1), default=256, show_default=True)
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name='training.SimNet.SimGenerator', z_dim=64, w_dim=128, bbox_dim=opts.bbox_dim, mapping_kwargs=dnnlib.EasyDict(), synthesis_kwargs=dnnlib.EasyDict())
    # c.G_kwargs = dnnlib.EasyDict(class_name='training.SimNet.SimGenerator', z_dim=512, w_dim=512, bbox_dim=opts.bbox_dim, mapping_kwargs=dnnlib.EasyDict(), synthesis_kwargs=dnnlib.EasyDict())
    ...
    print(opts.data)
    print(opts.data2)
    c.training_set_kwargs, dataset_name = init_dataset_kwargs(data=opts.data, bbox_dim=opts.bbox_dim)
    c.test_set_kwargs, _ = init_dataset_kwargs(data=opts.data2, bbox_dim=opts.bbox_dim, aug=False)
    ...
    c.G_kwargs.synthesis_kwargs.channel_base = opts.cbase
    c.G_kwargs.synthesis_kwargs.channel_max = opts.cmax
    c.G_kwargs.synthesis_kwargs.num_fp16_res = 4
    c.G_kwargs.synthesis_kwargs.conv_clamp = 256
    ...
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.loss3.ProjectedGANLoss')
    c.D_kwargs = dnnlib.EasyDict(
        class_name='training.discriminator.ProjectedDiscriminator',
        diffaug=True,
