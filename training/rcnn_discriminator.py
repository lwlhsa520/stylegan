...
class ResnetDiscriminator256(nn.Module):
    ...
    def forward(self, x, y=None, bbox=None):
        ...
        s_idx = ((bbox[:, 4] - bbox[:, 2]) < 128) * ((bbox[:, 3] - bbox[:, 1]) < 128)
...
class CombineDiscriminator256(nn.Module):
    def __init__(self, num_classes=81, **block_kwargs):
        super(CombineDiscriminator256, self).__init__()
        self.obD = ResnetDiscriminator256(num_classes=num_classes, input_dim=1)

    def forward(self, images, gt_bbox, mask=None, diff_aug=True):
        label, bbox = gt_bbox[:, :, 0].unsqueeze(-1), gt_bbox[:, :, 1:]
        ...
        bbox = torch.cat((label, bbox.float()), dim=2)
        if diff_aug:
            images, _, bbox = DiffAugment(images, bbox=bbox, policy='color,translation,cutout')

        label, bbox = bbox[:, :, 0].long().unsqueeze(-1), bbox[:, :, 1:]
