...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, masks=None, debug_percentile=None):
        assert isinstance(images, torch.Tensor) and images.ndim == 4
        if masks is not None:
            assert isinstance(masks, torch.Tensor) and masks.ndim == 4
        ...
        if G_inv is not I_3:
            ...
            if masks is not None:
                masks = torch.nn.functional.pad(input=masks, pad=[mx0,mx1,my0,my1], mode='reflect')
            G_inv = translate2d((mx0 - mx1) / 2, (my0 - my1) / 2) @ G_inv

            # Upsample.
            images = upfirdn2d.upsample2d(x=images, f=self.Hz_geom, up=2)
            if masks is not None:
                masks = upfirdn2d.upsample2d(x=masks, f=self.Hz_geom, up=2)
            ...
            if masks is not None:
                masks = grid_sample_gradfix.grid_sample(masks, grid)

            # Downsample and crop.
            images = upfirdn2d.downsample2d(x=images, f=self.Hz_geom, down=2, padding=-Hz_pad*2, flip_filter=True)
            if masks is not None:
                masks = upfirdn2d.downsample2d(x=masks, f=self.Hz_geom, down=2, padding=-Hz_pad*2, flip_filter=True)
