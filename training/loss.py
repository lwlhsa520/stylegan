...
class StyleGAN2Loss(Loss):
    ...
    def run_D(self, img, mask, sync):
        ...
        with misc.ddp_sync(self.D, sync):
            logits = self.D(img, mask, getVec=False)
        return logits
    ...
    def accumulate_gradients(self, phase, real_img, real_mask, real_bbox, gen_z, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, gen_mask, _gen_ws, _gen_ws2, gen_mid_masks = self.run_G(gen_z, real_bbox, sync=(sync and not do_Gpl)) # May get synced by Gpl.
                gen_logits = self.run_D(gen_img, gen_mask, sync=False)
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
                loss_Gmain = torch.nn.functional.softplus(-gen_logits) # -log(sigmoid(gen_logits))
                training_stats.report('Loss/G/loss', loss_Gmain)

                # gen_mid_masks = gen_mid_masks.sum(1, keepdim=True).clamp(max=1.0)
                # mask_loss = self.P(gen_img, real_img) + self.P(gen_mid_masks, F.adaptive_avg_pool2d(gen_mask, gen_mid_masks.shape[2:4])) + self.P(gen_mid_masks, F.adaptive_avg_pool2d(real_mask, gen_mid_masks.shape[2:4]))
                # mask_loss = self.P(gen_img, real_img) + self.P(gen_mask, real_mask) + self.P(gen_mid_masks.sum(dim=1, keepdim=True).clamp(-1.0, 1.0), F.adaptive_avg_pool2d(real_mask, gen_mid_masks.shape[2:4]))
                mask_loss =  self.P(gen_img, real_img) + self.P(gen_mask, real_mask) + self.P(gen_mid_masks.sum(dim=1, keepdim=True).clamp(max=1.0).mul(2.0) - 1.0, F.adaptive_avg_pool2d(real_mask, gen_mid_masks.shape[2:4]))
            with torch.autograd.profiler.record_function('Gmain_backward'):
                (gen_mask[:, 0, 0, 0]*0 + loss_Gmain + mask_loss).mean().mul(gain).backward()
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, gen_mask, _gen_ws, _gen_ws2, _gen_mid_masks= self.run_G(gen_z, real_bbox, sync=False)
                gen_logits = self.run_D(gen_img, gen_mask, sync=False) # Gets synced by loss_Dreal.
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                real_logits = self.run_D(real_img_tmp, real_mask_tmp, sync=sync)
