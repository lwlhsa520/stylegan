...
if __name__ == "__main__":
    ...
    # print(images.shape, labels.shape, masks.shape, bboxs.shape)
    # labels = my_label.unsqueeze(0).repeat(batch_size, 1, 1)
    ...
    # gen_imgs, gen_masks = Gnet(z, c, bboxs)
    # bmask = bbox_mask(device, bboxs, 64, 64)
    # gen_imgs, gen_masks, _, _, gen_mid_masks = Gnet(z, bboxs)
    gen_imgs, gen_masks, _, _, gen_mid_masks = G(z, bboxs)

    # for idx, (res, img) in enumerate(zip(g_imgs, images)):
    #     for idy, im in enumerate(g_imgs[res]):
    #         canvas_s.paste(PIL.Image.fromarray((im[0] * 127.5 + 128).clamp(0, 255).to(torch.uint8).cpu().numpy()).convert('L'), (W * idx, H * idy))
    #         canvas_s.paste(PIL.Image.fromarray((im[0] * 127.5 + 128).clamp(0, 255).to(torch.uint8).cpu().numpy()).convert('L'), (W * idx, H * idy))
    #         canvas_s.paste(PIL.Image.fromarray((im[0] * 127.5 + 128).clamp(0, 255).to(torch.uint8).cpu().numpy()).convert('L'), (W * idx, H * idy))
    #         canvas_s.paste(PIL.Image.fromarray((im[0] * 127.5 + 128).clamp(0, 255).to(torch.uint8).cpu().numpy()).convert('L'), (W * idx, H * idy))
    #         canvas_s.paste(PIL.Image.fromarray((im[0] * 127.5 + 128).clamp(0, 255).to(torch.uint8).cpu().numpy()).convert('L'), (W * idx, H * idy))
    #
    # canvas_s.save(f'./layers.png')
    # print(images/127.5 - 1)
    # print(gen_imgs.max(), gen_imgs.min(), gen_masks.max(), gen_masks.min())

    # d1, d2 = D(gen_imgs, c, bboxs, y)
    # print(torch.nn.functional.softplus(-d1))
    # print(torch.nn.functional.softplus(-d2.view(batch_size, -1).sum(-1, keepdim=True)))
    # # print(gen_imgs.dtype, images.dtype)
    # print(vgg_loss(gen_imgs, imgs))
    # print(Perceptual_loss(gen_imgs, imgs))
    ...
    print(gen_masks.shape, gen_imgs.shape, images.shape, masks.shape)
